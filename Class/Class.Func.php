...
class __fn__ {
	...
	public function chk_admin($sess, $admin) {
		if(empty($sess[$admin]['idx']) || empty($sess[$admin]['id']) || empty($sess[$admin]['name'])) {
			exit( $this->loca("/".strtolower($admin)."/login.html", "로그인 정보가 없습니다.\\n\\n다시 로그인 해 주세요") );
		}
	}
...
/*
	public function menu_sub($sm, $smd, $mm, $mmd, $ret = false) {
		$return = ($sm == $smd && $mm == $mmd) ? ' active_sub' : '';
		if($ret == false) { echo $return; } else { return $return; }
	}
*/
